...
function LawyerMap({ navigation, route }) {
  const { lawyerLatitude, lawyerLongitude ,lawyerName} = route.params;
  const [location, setLocation] = useState(null);
  const [region, setRegion] = useState({
    latitude: lawyerLatitude,
    longitude: lawyerLongitude,
    latitudeDelta: 0.01,
    longitudeDelta: 0.01,
  });
  ...
  return (
    <View style={{ alignItems: "center" }}>
      ...
      >{lawyerLatitude &&
        <Marker
          coordinate={{ latitude: lawyerLatitude, longitude: lawyerLongitude }}
          title="محكمه"
