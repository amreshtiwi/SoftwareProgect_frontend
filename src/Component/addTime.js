...
function AddTime({ visible, hideModal, date ,agendaData}) {
    ...
    <Portal>
      ...
      >
        ...
        <View style={styles.timeView}>
          <Pressable style={styles.timeViewComponent} onPress={showStartTimePicker}>
            <Input
              label={"من"}
              value={startTime}
              width={"95%"}
          ...
          <Pressable style={[styles.timeViewComponent,{alignItems:'flex-end'}]} onPress={showEndTimePicker}>
            <Input
              label={"إلى"}
              value={endTime}
              width={"95%"}
        ...
        {startTimeconflict || endTimeConflict ? (<Text style={{color:'red'}}>الموعد متعارض مع المواعيد الاخرى</Text>):null}
        {startTime !== "" && endTime !== "" && description !== "" && (<Btn value={"أحجز"}></Btn>)}
      </Modal>
    </Portal>
  );
}
